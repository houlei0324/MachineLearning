...
def preprocess(originfile,edgefile,vertexfile):
	...
	while 1:
		...
		if line[1] not in vitem.keys():
			...
			i = i + 1
		if t % 10 == 5 or t % 10 == 9:
			lines.append(str(vuser[line[0]]) + '\t' + str(vitem[line[1]]) + '\t' +line[3] + '\n')
		else:
			lines.append(str(vuser[line[0]]) + '\t' + str(vitem[line[1]]) + '\t' + line[3] + '\n')
		t += 1
		print str(vuser[line[0]]) + '\t' + str(vitem[line[1]]) + '\t' + line[3]
	print len(vitems)
	for lin in vs.keys():
...
if __name__ == '__main__':
	...
	originfile = path + 'smallnetflix.dat'
	edgefile = path + 'smallnetflix.e'
	vertexfile = path + 'smallnetflix.v'
